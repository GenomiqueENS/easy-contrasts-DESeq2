...
We collapse the technical replicates by summing their raw count for each gene. We display part of this matrix, meaning 5 genes and 5 samples. As for the raw counts, collapsed count data are integer values.
...
# Display
DESeq2::counts(dds)[top5_to_print, c(1:5)] %>%
...
```{r save_pool_norm_count_matrix_rds, eval = saveRDS, results = 'asis'}
cat("We also save the object in RDS format. The file name is the same. The extension is '.rds'.\n")

saveRDS(dds,
        file = stringr::str_replace(string = filename,
                                    pattern = "tsv$",
                                    replacement = "rds"))
```
