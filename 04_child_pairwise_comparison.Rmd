...
The MA plot represents the log<sub>2</sub> fold change of each gene (dot) as a function of their mean normalised expression value across all samples. Dots are colored based on the significance of the statistical test, using a significant threshold of **`r padjLimit`** for the adjusted p-value (`padj`).
...
ggplot2::ggplot(res, aes(x = baseMean, y = log2FoldChange,
  ...
  ggplot2::scale_color_manual(name = paste0("padj < ", padjLimit),
...
We label **`r length(genes_up)`** significantly over-regulated and **`r length(genes_dn)`** significantly down-regulated genes, considering an adjusted p-value of **`r padjLimit`**.

```{r volcano_prep_1, eval = is.null(correspPath), echo = is.null(correspPath)}
p = res %>%
...
```{r volcano_prep_2, eval = !is.null(correspPath), echo = !is.null(correspPath)}
res = res %>%
  dplyr::mutate(symbol = correspIdSymbol[ID, 2])
...
p = p +
  ...
  ggplot2::geom_hline(yintercept = -log10(padjLimit), lty = 2, col = "#A3ADB8") +
  ggplot2::geom_vline(xintercept = log2FCLimit, lty = 2, col = "#A3ADB8") +
  ggplot2::geom_vline(xintercept = -log2FCLimit, lty = 2, col = "#A3ADB8") +
  # Points
  ...
  ggplot2::geom_point(data = res %>%
                        dplyr::filter(log2FoldChange > -log2FCLimit &
                                        log2FoldChange < log2FCLimit),
                      mapping = aes(col = "Others")) +
  ggplot2::geom_point(data = res %>%
                        dplyr::filter(padj < padjLimit) %>%
                        dplyr::filter(log2FoldChange >= log2FCLimit),
...
p + ggrepel::geom_label_repel(data = ~ dplyr::filter(., ID %in% c(genes_up, genes_dn)),
...
```{r zscore_mat_2, eval = !is.null(correspPath), echo = !is.null(correspPath)}
...
matrix(zscore_mat[, c(1:2)],
  ...
  as.data.frame() %>%
  dplyr::mutate(Symbol = correspIdSymbol[rownames(.), 2]) %>%
...
In case a gene is expressed in a single sample, the normalisation outputs NaN values. We remove them.

```{r remove_NaN}
NaN_rows = (which(rowSums(is.nan(zscore_mat)) > 0))

if (length(NaN_rows) > 0) {
  nb_genes = nb_genes - length(NaN_rows)
  zscore_mat = zscore_mat[-(NaN_rows), ]
}
```
...
# Rownames
if (!is.null(correspPath)) {
  row_labels = correspIdSymbol[rownames(zscore_mat), 2]
} else {
  row_labels = rownames(zscore_mat)
}
...
ht = ComplexHeatmap::Heatmap(
  ...
  # Expression colors
  col = expressionColors(range(zscore_mat, na.rm = TRUE)),
