...
```{r save_processed_dds_rds, eval = saveRDS, results = 'asis'}
cat("## Save object\n\n")
cat("We save the dds object in RDS format.\n")

filename = paste0(prefix, projectName,
                  "-processed_dds_object.rds")

saveRDS(dds,
        file = filename)

cat(paste0("Output file name:<br>*", filename, "*\n"))
```
...
for(i in c(1:nrow(comparison_df))) {
  ...
  # Figure subtitle
  comparison_subtitle = stringr::str_replace_all(string = comparison_name,
                                                 pattern = "_",
                                                 replacement = " ")
